<div align="center">

<h1>Hi 👋, I'm Omar Osama</h1>
<h3>.NET Backend Developer | ITI Trainee 🚀</h3>

<img src="https://github.com/SP-XD/SP-XD/blob/main/images/dev-working_rounded.gif?raw=true" width="40%" />

</div>

---

## 🧠 About Me

* 🎓 Currently studying at **ITI (Information Technology Institute)**
* 💻 Focused on **Backend Development using .NET**
* 🏗️ Building scalable apps using:

  * ASP.NET Core MVC
  * Web API
  * Entity Framework Core
* 📚 Currently learning:

  * Advanced C#
  * LINQ
  * Angular (Frontend integration)
* 🧩 Strong understanding of:

  * OOP, SOLID Principles
  * Clean Architecture & Design Patterns
* 🗄️ Good experience with SQL Server & Database Design

---

## 🚀 Tech Stack

### 👨‍💻 Backend

![C#](https://img.shields.io/badge/C%23-239120?style=flat\&logo=c-sharp\&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat\&logo=dotnet\&logoColor=white)
![EF Core](https://img.shields.io/badge/EF%20Core-512BD4?style=flat)
![LINQ](https://img.shields.io/badge/LINQ-000000?style=flat)

### 🌐 Frontend

![Angular](https://img.shields.io/badge/Angular-DD0031?style=flat\&logo=angular\&logoColor=white)
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=flat)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=flat)
![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=flat)

### 🗄️ Database

![SQL Server](https://img.shields.io/badge/SQL%20Server-CC2927?style=flat\&logo=microsoftsqlserver\&logoColor=white)

### ⚙️ Tools

![Git](https://img.shields.io/badge/Git-E44C30?style=flat)
![Visual Studio](https://img.shields.io/badge/Visual%20Studio-5C2D91?style=flat)
![VS Code](https://img.shields.io/badge/VSCode-0078D4?style=flat)

---

## 📌 Featured Projects

### 🏋️ Gym Management System

* ASP.NET Core MVC Application
* 3-Tier Architecture (DAL / BLL / PL)
* Repository & Unit of Work Pattern
* Authentication & Authorization

### 📝 Examination System Database

* SQL Server Project
* Full DB Design (Students, Courses, Exams)
* Stored Procedures & Constraints

---

## 📊 GitHub Stats

<div align="center">
<img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight" />
</div>

---

## 📫 Contact Me

* 💼 LinkedIn: (put your link)
* 📧 Email: (your email)

---

## ⚡ Fun Fact

Coding late at night hits different 🌙
